...
This design adds overhead to each page table modification, which significantly reduces the performance of the guest OS. For this reason, as long as there is hardware support for them, nested page tables are the preferred way of isolating guest OS memory. Let's not forget also that without nested page tables the hypervisor has to maintain shadow page tables that translate the guest-virtual→guest-physical mappings the guest operating systems have in their page tables to host-physical mappings for every memory access. Using shadow page tables to detect when pages go stale sounds like it could save us from incurring the overhead from marking the guest OS page table region as read-only, but trapping on stale accesses incurs about just as much overhead. In this case, abandoning eager loading for lazy loading doesn't solve the problem; only hardware support does.
...
Without ballooning, the hypervisor itself would have to decide which pages to evict on behalf of the guest OS, and it would invariably make much less informed decisions (since it doesn't control the guest OS processes directly), which would degrade performance (e.g., it could evict a page that would then be immediately paged back in because it was in use by a process). When delegating a problem, pick a layer closest to it because it has the best available information for solving it.
